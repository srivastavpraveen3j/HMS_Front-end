...
@Component({
  ...
  templateUrl: './manageinward.component.html',
  styleUrl: './manageinward.component.css',
})
export class ManageinwardComponent {
  pathoareq: any[] = [];
                ...
                const hasPendingTest =
                  ...
                  req.testGroup.some(
                    (group: any) =>
                      ...
                      req.type === 'inpatientDepartment'
                  );

                // Filter for pathology requests with pending tests
                if (req.typeOfRequest === 'pathology' && hasPendingTest) {
  ...
  onSubmit(): void {
    ...
    this.testservice.postTestreq(payload).subscribe({
      next: (res) => {
        ...
        this.router.navigateByUrl('/pathologylayout/pathologylayout');
      },
      error: (err) => {
