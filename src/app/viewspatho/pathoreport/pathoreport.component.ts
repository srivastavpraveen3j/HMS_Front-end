...
@Component({
  selector: 'app-pathoreport',
  templateUrl: './pathoreport.component.html',
  styleUrls: ['./pathoreport.component.css'],
  imports : [CommonModule, RouterModule]
})
export class PathoreportComponent implements OnInit {
  ...
  loadPathologyTestGroupsFromPatients(): void {
    this.masterService.getTestreq().subscribe({
      next: (res: any[]) => {
        ...
        this.patho = res.filter(item => item.requestedDepartment === 'pathology');
