...
@Component({
  selector: 'app-radioheader',
  imports: [RouterModule, CommonModule, NotificationComponent],
  templateUrl: './radioheader.component.html',
  styleUrl: './radioheader.component.css'
})
...
ngOnInit() {
  ...
  this.logoService.logoUrl$.subscribe(url => {
   ...
   this.shapeSubscription = this.logoService.shapeConfig$.subscribe(
    ...
    this.radiologypermission = uhidModule?.permissions?.create === 1;
...
hasAnyDropdownAccess(): boolean {
  return (
    ...
    this.hasModuleAccess('pharmaceuticalInward',  'read') ||
     this.hasModuleAccess('sharedPatientCases', 'read') ||
