...
@Component({
  selector: 'app-walkinreport',
  imports: [ CommonModule,
      FormsModule,
      CanvasJSAngularChartsModule,
      LoaderComponent],
  templateUrl: './walkinreport.component.html',
  styleUrl: './walkinreport.component.css'
})
export class WalkinreportComponent {
title: String = '';
  subTitle: String = '';

  opdpathologyData: any[] = [];
  filteredData: any[] = [];
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params) => {
      this.title = params['title'] || 'OPD RADIOLOGY PATIENTS';
    });
  ...
  getOPDpharmaBill() {
    ...
    const fetchPage = (page: number) => {
      this.pharmaservice.getTestreq(page).subscribe({
        ...
        error: (error: any) => {
          console.error('Error fetching OPD patho data:', error);
  ...
  enrichPharmaWithUHID(pharmaList: any[]) {
    ...
    if (pharmaList.length === 0) {
      this.opdpathologyData = [];
      this.filteredData = [];
      this.isLoading = false;
      return;
    }

    for (const patho of pharmaList) {
      ...
      if (!uhidId || typeof uhidId !== 'string') {
        ...
        completed++;
        this.checkCompletion(completed, pharmaList.length, enrichedData);
        continue;
      }
  ...
  private checkCompletion(
    ...
    if (completed === total) {
      this.opdpathologyData = enrichedData;
      this.filteredData = [...this.opdpathologyData];
      this.applyDateRange();
      this.isLoading = false;
    }
  }
  ...
  applyDateRange() {
    ...
    switch (this.selectedRange) {
      case 'today':
        ...
        this.subTitle = ` - Today's OPD Pathology Report (${formatDate(
          startDate
        )})`;
        break;
      case 'week':
        ...
        this.subTitle = ` - This Week OPD Pathology Report (${formatDate(
          startDate
        )} to ${formatDate(new Date())})`;
        break;
      case 'month':
        startDate = new Date(today.getFullYear(), today.getMonth(), 1);
        this.subTitle = ` - This Month OPD Pathology Report (${formatDate(
          startDate
        )} to ${formatDate(today)})`;
        break;
      case 'year':
        ...
        endDate = today;
        this.subTitle = ` - This Year OPD Pathology Report (${formatDate(
          startDate
    ...
    this.filteredData = this.opdpathologyData.filter((bill) => {
    ...
    // console.log("🚀 ~ Filtered Patients:", this.filteredData);
  ...
  onSearch() {
    ...
    this.filteredData = this.opdpathologyData.filter((record) => {
  ...
  async downloadPDF() {
    ...
    try {
      ...
      pdf.save(
        `OPD_Pathology_Report_${this.selectedRange}_${
