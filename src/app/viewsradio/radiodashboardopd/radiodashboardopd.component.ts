...
@Component({
  selector: 'app-radiodashboardopd',
  imports: [
    RouterModule,
    CommonModule,
  ...
  templateUrl: './radiodashboardopd.component.html',
  styleUrl: './radiodashboardopd.component.css',
})
export class RadiodashboardopdComponent {
  ...
  filterForm!: FormGroup;
   ...
   buildFilters(): any {
     ...
     // Source type filter (only OPD)
     filters.sourceType = 'opd';
   ...
   viewPatient(recordId: string): void {
     ...
     this.ipdService.getRadioInwardById(recordId).subscribe({
       next: (response) => {
         console.log('✅ Patient details loaded:', response);
   ...
   printReport(recordId: string): void {
     ...
     this.ipdService.getRadioInwardById(recordId).subscribe({
       next: (response) => {
         console.log('✅ Patient details loaded for printing:', response);
         this.generatePrintableReport(response);
       },
       error: (err) => {
