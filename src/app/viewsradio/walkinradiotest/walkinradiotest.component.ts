...
@Component({
  ...
  templateUrl: './walkinradiotest.component.html',
  styleUrl: './walkinradiotest.component.css'
})
export class WalkinradiotestComponent {
    ...
    searchForm!: FormGroup;
      ...
      ) // private multideptservice : PharmaService
      {
        this.pathoinward = this.fb.group({
          ...
          remarks: [''],
          typeOfRequest: ['radiology'], // assuming always pathology here
                ...
                if (Array.isArray(patient.departmentreqlists)) {
                  patient.departmentreqlists.forEach((req: any) => {
                    const hasPendingTest =
                      ...
                      req.testGroup.some(
                        (group: any) => group?.status?.toLowerCase() === 'radiology'
                      );

                    // Filter for pathology requests with pending tests
                    if (
                      req.typeOfRequest === 'radiology' &&
                      hasPendingTest &&
      ...
      onSubmit(): void {
        ...
        const payload = {
          ...
          total: formValues.total,
            isWalkIn: true,
              walkInPatient: {
          name: formValues.walkInPatient.name,
          age: formValues.walkInPatient.age,
          gender: formValues.walkInPatient.gender,
          mobile_no : formValues.walkInPatient.mobile_no,
          referenceDoctor : formValues.walkInPatient.referenceDoctor,
          address : formValues.walkInPatient.address
        },
        };
        ...
        this.testservice.postTestreq(payload).subscribe({
          next: (res) => {
            ...
            this.router.navigateByUrl('/radiologylayout/radiologylayoutwalkin');
          },
          error: (err) => {
   ...
   selectAssignTest(test: any): void {
    ...
    // Auto-populate lower section like onRowSelect()
    this.selectedTestGroupName = test.testGroup?.trim();
    this.selectedTestGroupStatus = test.status || 'pending'; // fallback
    this.selectedTestParameters = test.testParameters || [];

    this.pathoinward.patchValue({
      total: test.price || 0
    });

    console.log('🧪 Selected Test Group:', test);
  }
  ...
  removeAssignTest(test: any): void {
    ...
    // Clear lower section if the removed one was selected
    if (this.selectedTestGroupName === test.testGroup) {
      this.selectedTestGroupName = '';
      this.selectedTestGroupStatus = '';
      this.selectedTestParameters = [];
      this.pathoinward.patchValue({ total: 0 });
    }
  }
