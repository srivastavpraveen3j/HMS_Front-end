...
import { WalkinopdpharmaComponent } from "../../component/opdcustomfiles/walkinopdpharma/walkinopdpharma.component";
import { IndianCurrencyPipe } from '../../pipe/indian-currency.pipe';

@Component({
  selector: 'app-awalkinpharmadashboard',
   imports: [
    CommonModule,
    ...
    WalkinopdpharmaComponent,
    IndianCurrencyPipe
],
  templateUrl: './awalkinpharmadashboard.component.html',
  styleUrl: './awalkinpharmadashboard.component.css'
})
export class AwalkinpharmadashboardComponent {
 doctors: any[] = [];
  ...
  userPermissions: any = {};
  pharmaPermissions: any = {};
  ipdpharmaPermissions: any = {};
 ...
 ngOnInit(): void {
  ...
  const uhidModule = allPermissions.find((perm: any) => perm.moduleName === 'pharmaceuticalInward');
    ...
    const ipdpharmaModule = allPermissions.find(
      (perm: any) => perm.moduleName === 'ipdpharmaceuticalRequestList'
    );
  this.userPermissions = uhidModule?.permissions || {};
  this.pharmaPermissions = pharmaModule?.permissions?.read;
  this.ipdpharmaPermissions = pharmaModule?.permissions?.read;
  ...
  this.pharmaservice.getPharmareq().subscribe({
    next: (res) => {
      if (!Array.isArray(res.data)) {
        console.warn('Unexpected response format from getPharmareq:', res.data);
        return;
      }
      ...
      console.log('Filtered Walk-In Patients:', this.pharma);

      // No UHID enrichment needed for walk-ins
      this.applyFilters(); // ✅ Direct filter
    },
    error: (err) => {
      console.error('Error fetching pharma requests:', err);
    }
  });
}
  ...
  baseList = baseList.filter((data) => {
    const patient = data.walkInPatient?.name.toLowerCase() || '';

    return patient.includes(text);
  });
