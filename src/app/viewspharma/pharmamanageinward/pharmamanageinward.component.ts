...
import { debounceTime, firstValueFrom, of, switchMap, combineLatest, catchError } from 'rxjs';
...
@Component({
  selector: 'app-pharmamanageinward',
  standalone: true,
  imports: [RouterModule, CommonModule, ReactiveFormsModule, FormsModule, IndianCurrencyPipe,PharmapartpaymentComponent],
  templateUrl: './pharmamanageinward.component.html',
  styleUrl: './pharmamanageinward.component.css',
})
export class PharmamanageinwardComponent {
  pharmareq: any[] = [];
  ...
  selectPatient(patient: any): void {
    ...
    const pendingRequest = patient.pharmaceuticalrequestlists?.find(
      (req: any) => req.status === 'pending' && req.patientType === 'outpatientDepartment'
    );
    ...
    this.doctorservice.getPharmareq(1, 1000).subscribe((res) => {
      ...
      res.forEach((patient: any) => {
        patient.pharmaceuticalrequestlists.forEach((req: any) => {
          if (req.status === 'pending' && req.patientType === 'outpatientDepartment') {
            flattened.push({
              patient_name: patient.patient_name,
              uhid: patient.uhid,
              age: patient.age,
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      this.router.navigateByUrl('/pharmalayout/opdpharmalayout');
