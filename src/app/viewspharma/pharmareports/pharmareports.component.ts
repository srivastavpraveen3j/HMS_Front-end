...
@Component({
  ...
  templateUrl: './pharmareports.component.html',
  styleUrl: './pharmareports.component.css',
})
export class PharmareportsComponent implements OnInit, AfterViewInit {
  title: String = '';
  subTitle: String = '';
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params) => {
      this.title = params['title'] || 'OPD PHARMA PATIENTS';
    });
  ...
  const fetchPage = (page: number) => {
    this.pharmaservice.getPharmareq(page).subscribe({
      next: (response: any) => {
        // Robust extraction for paginated or array responses
        const array = Array.isArray(response)
          ? response
          : response.data || response.items || response.records || [];

        // Only include walk-in OPD records!
  ...
  enrichPharmaWithUHID(pharmaList: any[]) {
    // console.log("🚀 ~ PharmareportsComponent ~ enrichPharmaWithUHID ~ enrichedData:",pharmaList)
    ...
    // console.log("🚀 ~ PharmareportsComponent ~ enrichPharmaWithUHID ~ pharmaList:", pharmaList)
    for (const pharma of pharmaList) {
        // console.log("Looping pharma:", pharma);
      const uhidId = pharma.uniqueHealthIdentificationId._id;
      console.log("🚀 ~ PharmareportsComponent ~ enrichPharmaWithUHID ~ uhidId:", uhidId)
      // console.log("🚀 ~ PharmareportsComponent ~ enrichPharmaWithUHID ~ pharma:", pharma)
      ...
      this.uhidservice.getUhidById(uhidId).subscribe({
        next: (res: any) => {
          ...
          // console.log("🚀 ~ PharmareportsComponent ~ enrichPharmaWithUHID ~ patient:", patient)
