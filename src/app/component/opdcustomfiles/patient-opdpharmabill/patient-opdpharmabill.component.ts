...
@Component({
  selector: 'app-patient-opdpharmabill',
    imports: [CommonModule, RouterModule, LetterheaderComponent],
  templateUrl: './patient-opdpharmabill.component.html',
  styleUrl: './patient-opdpharmabill.component.css'
})
export class PatientOpdpharmabillComponent {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['patientData'] && this.patientData?.length) {
      ...
      // Fetch medicines when patient data is available
      this.fetchMedicines();
    }
  }
  ...
  // Helper function to get medicine details from inventory
  // private getMedicineDetails(medicineName: string) {
  ...
  print() {
    ...
    const billNo = `PP${Date.now().toString().slice(-6)}`;
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>PP MANIYA PHARMACY - ${patient.uhid}</title>
          <style>
            @page {
              size: A4;
              margin: 6mm;
          ...
          <div class="bill-container">
            ...
            <div class="customer-info">
              <div>
                ...
                ${patient?.uniqueHealthIdentificationId?.patient_name || 'N/A'}<br>
                UHID: ${patient?.uniqueHealthIdentificationId?.uhid || 'N/A'}
              </div>
              <div style="text-align: right;">
                <strong>Date:</strong> ${currentDate}<br>
                <strong>Time:</strong> ${currentTime}<br>
                <strong>Bill No:</strong> ${patient.inwardSerialNumber}
              </div>
            </div>
                ...
                <div style="margin-top: 6px;">
                  ...
                  ${packages.map((pkg: any) => `
                    <div style="font-size: 6px; margin: 1px 0;">
                      • ${pkg.medicineName}: ${pkg.dosageInstruction || 'Before Meal Morning Night'}
                    </div>
                  `).join('')}
                </div>
              </div>
...
//     const p = this.patientData[0];
//     if (!p) return 'N/A';
//     const c = Number(p.cashAmount || 0);
//     const u = Number(p.upiAmount || 0);
...
//       if (u > 0) modes.push(`UPI ₹${u}${p.transactionId ? ' (Txn: ' + p.transactionId + ')' : ''}`);
...
//       return 'Card ₹' + card;
//     } else if (u > 0) {
//       return 'UPI ₹' + u + (p.transactionId ? ' (Txn: ' + p.transactionId + ')' : '');
//     }
//     return 'Cash';
//   }
