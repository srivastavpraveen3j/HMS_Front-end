...
@Component({
  selector: 'app-radiologyreqopdlist',
  imports: [
    CommonModule,
    RouterModule,
    TestComponent,
  ...
  templateUrl: './radiologyreqopdlist.component.html',
  styleUrl: './radiologyreqopdlist.component.css',
})
export class RadiologyreqopdlistComponent {
  ...
  //       this.pathologyData = pharmaRequests;
  //       console.log("🚀 Pharma Requests:", this.pathologyData);
  //     },
  //     error: (err) => {
  ...
  edittest(pathoid: string) {
    console.log(
      '🚀 ~ PathologyreqlistComponent ~ edittest ~ pathoid:',
      pathoid
    );

    this.router.navigate(['/doctor/pathologyreq'], {
      queryParams: { _id: pathoid },
    });
  }

  async deletetest(pathoid: string) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.doctorservice.deletereuesttestapis(pathoid).subscribe({
          next: (res) => {
            Swal.fire({
              icon: 'success',
              title: 'Deleted!',
