...
@Component({
  selector: 'app-radiologyreqopd',
  imports: [CommonModule, RouterModule, ReactiveFormsModule, FormsModule],
  templateUrl: './radiologyreqopd.component.html',
  styleUrl: './radiologyreqopd.component.css',
})
export class RadiologyreqopdComponent {
  radiology: FormGroup;
...
this.routes.queryParams.subscribe((params) => {
  ...
  if (patientid) {
    this.loadOpdRadioreqisByPatientId(patientid);
  }
});
    ...
    const nameControl = this.radiology.get('patient_name');
    const uhidControl = this.radiology.get('uhid');
      ...
      next: (res) => {
        ...
        for (const patient of allPatients) {
          ...
          for (const req of deptRequests) {
            if (req._id === testid && req.typeOfRequest === 'radiology') {
  ...
  onPatientInput(): void {
    const searchTerm = this.radiology.get('patient_name')?.value?.trim() || '';
    ...
    if (testid) {
      ...
      this.doctorservice.updatereuesttestapis(testid, payload).subscribe({
        next: (res) => {
          ...
          this.router.navigate(['/doctor/opdradiologyreqlist']);
        },
        error: (err) => {
      ...
      this.doctorservice.postreuesttestapis(payload).subscribe({
        next: (res) => {
          ...
          this.radiology.reset();
          this.router.navigateByUrl('/doctor/opdradiologyreqlist');
          this.selectedAssignTests = []; // Reset selected tests if needed
        },
        error: (err) => {
          console.error('Error submitting radiology request:', err);
          Swal.fire({
            icon: 'error',
            title: 'Submission Failed',
            text: 'An error occurred while submitting the radiology request.',
            customClass: {
              popup: 'hospital-swal-popup',
              title: 'hospital-swal-title',
              htmlContainer: 'hospital-swal-text',
  ...
  loadOpdRadioreqisByPatientId(patientId: string) {
