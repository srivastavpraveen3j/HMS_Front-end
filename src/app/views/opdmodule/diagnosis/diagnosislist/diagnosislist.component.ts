...
@Component({
  ...
  styleUrl: './diagnosislist.component.css',
})
        ...
        this.diagnosis = patientList
          .filter(
            (patient: any) =>
              ...
              patient.diagnosis.type === 'outpatientDepartment'
          )
          .map((patient: any) => {
        ...
        this.doctorservice.getVitals().subscribe({
          next: (res: any) => {
            this.vitals = res.filter(
              (vital: any) => vital.vitals?.[0]?.outpatientCaseId === this.selectedPatient?.[0]?.outpatientcases?._id
            );
