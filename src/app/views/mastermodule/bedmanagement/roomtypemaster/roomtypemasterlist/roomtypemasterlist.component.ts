...
@Component({
  selector: 'app-roomtypemasterlist',
  imports: [RouterModule, CommonModule, ReactiveFormsModule],
  templateUrl: './roomtypemasterlist.component.html',
  styleUrl: './roomtypemasterlist.component.css'
})
export class RoomtypemasterlistComponent {
...
// load permissions
  ...
  loadRoomtype(): void {
    ...
    this.bedwardroomservice.getroomtype(this.currentPage, limit, search).subscribe(res => {
      this.wards = res.roomTypes;
  ...
  editWard(wardId: string) {
    ...
    this.router.navigate(['/master/roomtypemaster'], { queryParams: { _id: wardId } });
  }
...
async deleteWard(wardId: string) {
  ...
  if (!wardId) {
    console.error("Room Type ID is required for deletion");
    return;
  }
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      this.bedwardroomservice.deleteroomType(wardId).subscribe({
        next: () => {
          Swal.fire({
            icon: 'success',
            title: 'Deleted!',
            text: 'Room Type has been deleted successfully.',
            position: 'top-end',
            toast: true,
            timer: 3000,
        ...
        error: (err) => {
          ...
          Swal.fire({
            ...
            text:err?.error?.message || 'There was an error deleting the Room Type.',
            customClass: {
              popup: 'hospital-swal-popup',
              title: 'hospital-swal-title',
              htmlContainer: 'hospital-swal-text',
