...
@Component({
  selector: 'app-radaitiontestparameter',
  imports: [CommonModule, RouterModule, ReactiveFormsModule],
  templateUrl: './radaitiontestparameter.component.html',
  styleUrl: './radaitiontestparameter.component.css'
})
export class RadaitiontestparameterComponent {
...
// load permissions
  ...
  loadMedicaltest(medicaltestId : string){
    this.masterService.getMedicaltest().subscribe((response : any)=>{
  ...
  if (medicaltestId) {
    ...
    this.masterService.updateMedicaltest(medicaltestId, formData).subscribe({
      next: () => {
        ...
        this.router.navigateByUrl('/radiationlayout/radiationtestparameterlist');
      },
      error: (error) => {
